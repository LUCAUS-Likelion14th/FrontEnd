...
const mockBooths: BoothLikeItem[] = [
  ...
  { booth_id: 2, location_id: 2, booth_image: "/img.png", booth_name: "부스 이름", booth_owner: "단체 이름", booth_location: "해방광장", is_liked: true, like_count: 52 },
...
const mockFoodtrucks: FoodLikeItem[] = [
  ...
  { food_id: 2, location_id: 1, image: "/img.png", food_name: "푸드트럭 이름", best_food: "대표 메뉴", location: "해방광장", is_liked: true, like_count: 5 },
  { food_id: 3, location_id: 1, image: "/img.png", food_name: "푸드트럭 이름", best_food: "대표 메뉴", location: "해방광장", is_liked: true, like_count: 5 },
  { food_id: 4, location_id: 1, image: "/img.png", food_name: "푸드트럭 이름", best_food: "대표 메뉴", location: "해방광장", is_liked: true, like_count: 5 },
  { food_id: 5, location_id: 2, image: "/img.png", food_name: "푸드트럭 이름", best_food: "대표 메뉴", location: "해방광장", is_liked: true, like_count: 6 },
  { food_id: 6, location_id: 1, image: "/img.png", food_name: "푸드트럭 이름", best_food: "대표 메뉴", location: "해방광장", is_liked: true, like_count: 15 },
  { food_id: 7, location_id: 3, image: "/img.png", food_name: "푸드트럭 이름", best_food: "대표 메뉴", location: "해방광장", is_liked: true, like_count: 5 },
  { food_id: 8, location_id: 1, image: "/img.png", food_name: "푸드트럭 이름", best_food: "대표 메뉴", location: "해방광장", is_liked: true, like_count: 35 },
]
...
export default function LikesPage() {
  ...
  return (
    <div>
      {/* 고정 헤더 */}
      <div className="fixed top-14 left-0 right-0 bg-white z-10 px-4">
        <div className="flex items-center gap-1 pb-5 pt-2.5">
        ...
        <div className="relative">
          <div className="flex">
            <button
              ...
              className={`flex-1 text-center pb-3 text-base ${
      ...
      <div className="pt-45 px-4 pb-24">
        <div className="grid grid-cols-2 gap-4">
            {tab === "booth"
            ? mockBooths.map((booth) => (
                <Link href={`/booth/${booth.booth_id}`} key={booth.booth_id} className="block">
                    ...
                    <div className="absolute bottom-2.25 left-1.75 right-1.75 flex items-end justify-between">
                        ...
                        <span className="text-base font-semibold leading-tight line-clamp-1">{booth.booth_name}</span>
                        <span className="text-xs leading-tight">{booth.booth_owner}</span>
                        <span className="text-xs leading-tight">{booth.booth_location}</span>
                        </div>
            ...
            : mockFoodtrucks.map((truck) => (
                <Link href={`/foodtruck/${truck.food_id}`} key={truck.food_id} className="block">
                    ...
                    <div className="absolute bottom-[9px] left-1.75 right-1.75 flex items-end justify-between">
                        <div className="flex flex-col gap-1">
                        <span className="text-base font-semibold leading-tight line-clamp-1">{truck.food_name}</span>
                        <span className="text-xs leading-tight">{truck.best_food}</span>
                        <span className="text-xs leading-tight">{truck.location}</span>
                        </div>
                        <div className="flex flex-col items-center gap-1 shrink-0">
                        {truck.is_liked ? (
                            <FaHeart size={24} className="text-primary" />

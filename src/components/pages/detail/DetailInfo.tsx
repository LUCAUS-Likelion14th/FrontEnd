...
export default function DetailInfo({
  ...
  const groups: LocationGroup[] = locationGroups ?? (location !== undefined ? [{ location, dates: date ?? [] }] : []);
        ...
        {groups.map((group, i) => (
          <div key={i} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <span className="text-sm break-keep pl-[19px]">{group.location}{group.locationId ? ` #${group.locationId}` : ""}</span>
            </div>
